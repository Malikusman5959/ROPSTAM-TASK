...
import { updateCategory } from '../fetchApi';
    ...
    const { values, errors, touched, handleBlur, handleChange, handleSubmit, isSubmitting } = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                ...
                let result = await updateCategory(searchParams.get('id'), values.categoryName);
        ...
        <>
            <form onSubmit={handleSubmit} className="flex flex-col justify-center w-[500px] py-8 bg-[#70707038] rounded-3xl">
                ...
                <div className="text-xl cursor-pointer flex flex-col justify-center items-center mt-5 md:mt-0">
                    <h1 className="uppercase font-semibold text-3xl text-white m-2 mb-4 tracking-widest">Update Category</h1>
                </div>
                ...
                <div className="text-center">
                    <button type='submit' disabled={isSubmitting} className="uppercase w-[350px] px-[50px] py-[7px] rounded-md text-white bg-blue-900 hover:bg-blue-800  font-medium ">Update</button>
                </div>
                {/* home page redirection */}
                <div className='text-white my-2 text-center'>
                    <button onClick={() => { navigate('/categories') }} disabled={isSubmitting} className="uppercase w-[350px] px-[50px] py-[7px] rounded-md text-white bg-blue-900 hover:bg-blue-800  font-medium "> {'<'} Back</button>
                </div>
            </form>
        </>
    )
